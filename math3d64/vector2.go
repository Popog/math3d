...
func MakeVector2(v ...float64) (r Vector2) {
	copy(r[:], v)
	return
}
...
func (v *Vector2) ScalarMultiplyThis(scalar float64) {
	for i := range v {
		v[i] *= scalar
	}
}

func (v1 Vector2) Dot(v2 Vector2) (r float64) {
	for i := range v1 {
		r += v1[i] * v2[i]
	}
	return
...
func (v Vector2) LengthSq() (m float64) {
	return v.Dot(v)
}
...
func (v1 Vector2) ApproxEquals(v2 Vector2, ε float64) bool {
	for i := range v1 {
