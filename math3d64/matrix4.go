...
// This is a 4x4 matrix of float64, stored in row major
type Matrix4 [4*4]float64

// Make a new matrix from the array
func MakeMatrix4V(v []float64, rowMajor bool) (r Matrix4) {
...
func (m Matrix4) ApproxEquals(q Matrix4, ε float64) bool {
	for i := 0; i < len(m); i++ {
...
func (m Matrix4) ScalarMultiply(scalar float64) Matrix4 {
	for i := 0; i < len(m); i++ { m[i] *= scalar }
	return m
}

func (m *Matrix4) ScalarMultiplyThis(scalar float64) {
