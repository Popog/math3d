...
type Quaternion [4]floatType

func MakeQuaternion(s, x, y, z floatType) Quaternion {
	return Quaternion{s, x, y, z}
}

func MakeQuaternionV(v []floatType) Quaternion {
...
func MakeQFromAxisAngle(axis Vector3, angle floatType) Quaternion {
...
func (q Quaternion) W() *floatType {
	return &q[0]
}

func (q Quaternion) X() *floatType {
	return &q[1]
}

func (q Quaternion) Y() *floatType {
	return &q[2]
}

func (q Quaternion) Z() *floatType {
	return &q[3]
}
...
func (m Quaternion) MultiplyS(scalar floatType) Quaternion {
	return Quaternion{m[0] * scalar, m[1] * scalar, m[2] * scalar, m[3] * scalar}
}

func (m Quaternion) DivS(scalar floatType) Quaternion {
	return Quaternion{m[0] / scalar, m[1] / scalar, m[2] / scalar, m[3] / scalar}
}
...
func (m Quaternion) Magnitude() floatType {
...
func (m Quaternion) FromAxisAngle(axis Vector3, angle floatType) Quaternion {
	// assert:  axis[] is unit length
	//
	// The quaternion representing the rotation is
	//   q = cos(A/2)+sin(A/2)*(x*i+y*j+z*k)
...
func (m Quaternion) ToAxisAngle() (axis Vector3, angle floatType) {
	// The quaternion representing the rotation is
	//   q = cos(A/2)+sin(A/2)*(x*i+y*j+z*k)
...
func (q Quaternion) Length() floatType {
	return Sqrtf(q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3])
}

func (q Quaternion) SquaredLength() floatType {
	return q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3]
}

func (q Quaternion) Normalize(ε floatType) (Quaternion, bool) {
...
func (q1 Quaternion) Dot(q2 Quaternion) floatType {
...
func Slerp(t floatType, p, q Quaternion) (r Quaternion) {
	...
	if Fabsf(angle) >= internalε {
		...
		r[0] = floatType(coeff0*p[0] + coeff1*q[0])
		r[1] = floatType(coeff0*p[1] + coeff1*q[1])
		r[2] = floatType(coeff0*p[2] + coeff1*q[2])
		r[3] = floatType(coeff0*p[3] + coeff1*q[3])
	} else {
		r[0] = p[0]
		r[1] = p[1]
		r[2] = p[2]
...
// t is the interpolation value from 0. to 1.
// ------------------------------------
func (m Quaternion) NLerp(t floatType, a, b Quaternion) Quaternion {
	w1 := 1.0 - t
...
*/
func (a Quaternion) ApproxEquals(b Quaternion, ε floatType) bool {
	for i := 0; i < 4; i++ {
		delta := Fabsf(a[i] - b[i])
		if delta > ε {
