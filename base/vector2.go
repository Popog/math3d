...
type Vector2 [2]floatType

func MakeVector2(v ...floatType) (r Vector2) {
	copy(r[:], v)
	return
}
...
func (v Vector2) ScalarMultiply(scalar floatType) Vector2 {
	v.ScalarMultiplyThis(scalar)
	return v
}
// In place scalar multiplication
func (v *Vector2) ScalarMultiplyThis(scalar floatType) {
	for i := range v {
		v[i] *= scalar
	}
}

func (v1 Vector2) Dot(v2 Vector2) (r floatType) {
	for i := range v1 {
		r += v1[i] * v2[i]
	}
	return
...
func (v1 Vector2) ApproxEquals(v2 Vector2, ε floatType) bool {
	for i := range v1 {
